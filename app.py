...
st.markdown(
    ...
    e selecione a coluna de tratamento e uma coluna numérica para gerar
...
def freedman_diaconis_bins(series: pd.Series) -> int | None:
...
def compute_summary(df: pd.DataFrame, col_treat: str, col_y: str) -> pd.DataFrame:
    """Resumo estatístico por tratamento."""
    resumo = (
...
def create_plot(
    ...
    # Boxplot por tratamento (apenas contínuo)
    ...
    # Barras – frequência por valor (para escala discreta)
...
if not numeric_cols:
    st.warning(
        "Não há colunas numéricas disponíveis após a conversão e exclusões.\n"
        "Verifique a linha do cabeçalho e o que foi excluído."
    )
    st.stop()

col_y = st.sidebar.selectbox(
    "Coluna numérica para analisar",
    numeric_cols,
)

# impedir que usuário escolha a mesma coluna duas vezes
if col_treat == col_y:
    st.error(
        "A coluna de tratamento e a coluna numérica para análise não podem ser a mesma. "
...
# 7. Filtros dinâmicos
# ==============================
df_filtered = df_analysis[[col_treat, col_y]].copy()
...
# se, por algum motivo, vier DataFrame, usa só a primeira coluna
...
# Filtro por faixa de valores da variável numérica
...
# ==============================
# 8. Resumo estatístico e tipo de dado
# ==============================
